...
# Metodi e casi
methods = [
    "No Resampling", "Random Undersampling", "Random Undersampling 10d", "NearMiss v3 (k=1)", "NearMiss v3 (k=3)",
    "NearMiss v3 (k=5)", "NearMiss v3 (k=10)", "NearMiss v3 (k=25)", "NearMiss v3 (k=50)",
    "CNN Undersampling", "ENN Undersampling", "Cluster Centroids Undersampling", "TomekLinks Undersampling",
    "Random Oversampling", "SMOTE Oversampling", "ADASYN Oversampling", "SVMSMOTE Oversampling"
]

cases = ["Case 1", "Case 2", "Case 3", "Case 4"]

# F1-score dati (in ordine corrispondente ai metodi e casi)
f1_scores = [
    [0.572, 0.663, 0.588, 0.576],
...
def plot_heatmap_centered_red_green(df, center_values, ax, cbar_label):
    normed_data = pd.DataFrame(index=df.index, columns=df.columns, dtype=float)
    for col in df.columns:
        col_vals = df[col]
        center = center_values[col]
        normed_data[col] = col_vals - center

    vlim = np.abs(normed_data.values).max()

    sns.heatmap(normed_data, annot=False, fmt="", cmap="RdYlGn",
                center=0, vmin=-vlim, vmax=vlim, cbar_kws={'label': cbar_label},
                ax=ax)

    for i, method in enumerate(df.index):
        for j, case in enumerate(df.columns):
            val = df.loc[method, case]
            if i == 0:
    ...
    ax.set_xticklabels(df.columns, rotation=0, ha='right', fontsize=14)
...
center_f1 = f1_df.loc["No Resampling"]
center_mcc = mcc_df.loc["No Resampling"]

# Plot F1-score
plot_heatmap_centered_red_green(
    f1_df, center_f1, axes[0], "F1-score deviation")
axes[0].xaxis.set_ticks_position('top')
axes[0].xaxis.set_label_position('top')
axes[0].set_title("F1-score", fontsize=20, fontweight='bold', pad=20)
axes[0].set_ylabel('Methods', fontsize=14, fontweight='bold')

# Plot MCC
plot_heatmap_centered_red_green(mcc_df, center_mcc, axes[1], "MCC deviation")
axes[1].xaxis.set_ticks_position('top')
axes[1].xaxis.set_label_position('top')
axes[1].set_title("MCC", fontsize=20, fontweight='bold', pad=20)
axes[1].set_yticklabels([])
